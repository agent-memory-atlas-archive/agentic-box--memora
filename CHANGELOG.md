...
- `scripts/preview_backfill_47.py` is a READ-ONLY preview of the #47 backfill with an explicit approval list (every row `"approved": false`); it creates nothing and may refuse: a local WAL store with sidecars (a writer process such as the server) is refused, with a non-zero exit. It supports local SQLite and D1 stores only: an S3 cloud store is refused before anything is opened, because its connect can sync the local cache. Contradictions (stored section vs the project the old typed-tag rule gave) get a proposed target only from agreeing non-typed evidence (metadata.project, non-typed tags, section); keyword-only rows only where the section came from content keywords; everything else is `needs-human`. A proposal sets `metadata.project` and re-prefixes the typed tags, with a `<project>/<subsection>` marker tag listed as an alternative.
