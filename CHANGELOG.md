...
- `memora-server` now pins uvicorn to `http="h11"`, `loop="asyncio"`, `h11_max_incomplete_event_size=16384` (the stated body bound -- 64 KiB plus one transport read -- depends on it; that read was measured at 262144 bytes with uvicorn 0.53.0, the image's version, on CPython 3.12.8, the closest runtime available for the measurement (the image runs CPython 3.12.14); earlier data points: 262144 with uvicorn 0.42.0 on CPython 3.12.8 and 3.13.1; `scripts/measure_asgi_body_messages.py`, which also runs inside the image: `docker run --rm -i memora:latest python - < scripts/measure_asgi_body_messages.py`). A local SQLite store's directory is now created on its first writing connection rather than when the backend object is built.
