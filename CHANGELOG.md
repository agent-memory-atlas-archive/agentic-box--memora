...
- **Export/import:** `memory_export` records gain a `system_tags` field (the typed tags memora applied); `memory_import` re-applies them through the same type-bound validation, so an export restores under the default policy. **`memory_import` with `strategy="replace"` now prepares every entry before deleting anything, and aborts with nothing deleted (`replaced: false`) if any entry fails**; it used to clear the store first and then reject entries one by one. Writes are atomic on local SQLite (the delete and every insert are one transaction; a write error rolls back to the unchanged store, `replaced: false`). **On D1 an import is not atomic.** A replace clears the store in stages (crossrefs, embeddings, FTS, then memories last), each retried; a stage that still fails stops with `replaced: "partial"` and `clear_stage`, every memory is still present until the last stage, and re-running the same replace simply continues. Rows are then written in order, each retried a bounded number of times; every INSERT carries a random per-import marker (stripped once the row is complete), and only a row carrying that marker is ever adopted after a lost response or removed after a failed write, so a pre-existing memory with the same text is never touched. The import stops at the first row that still fails, with `failed` and `written_ids` (and, for a replace, `replaced: "partial"`); the store then holds exactly those new rows, and the recovery path is re-running the import from the export file. A replace is never reported as done with errors. The failed row's cleanup deletes the memory row before its vector and then checks that it is gone; if it is not, the result lists it in `orphan_ids` (id and marker) and no longer claims the store holds exactly `written_ids`. **At most one D1 import runs on a store at a time:** every strategy (replace, merge, append) takes the store's single import lease (new table `import_lease`, one row) before the sweep, the merge read and preparation; a second import on the same store fails fast with `error: "import_in_progress"` and writes nothing (it does not wait). The lease is valid 30 minutes and renewed at least every 30 s; renewal only extends an unexpired lease this import owns, so an expired lease is never resurrected. Ownership is proven by a fresh read before each replace clear stage, before each row's INSERT, before completing it and before counting it; an import that has lost its lease stops at once, writes nothing further, keeps `written_ids` exact and reports a row it left marked in `left_marked` (for the sweep) or one it completed just before in `unconfirmed_ids`. Each row's marker carries the time of that row's first INSERT. The steps after the row phase (restoring a replace's embedding-integrity baseline, then rebuilding cross-references, including the rebuild's lazy embedding backfill) also run only under proven ownership: they are skipped entirely after a partial or lost row phase (`post_write: "skipped"`), and fenced before each step and before every write of the rebuild (each backfill and each crossref chunk), stopping with `post_write: "incomplete"` if the lease is lost; `post_write_note` then says to run `memory_rebuild_crossrefs`. The lease is released only after these steps. The row's completion is a compare-and-set on its marker followed by a read-back: a row found removed is inserted again, and a row is never counted as imported until it is verified complete. Rows left marked by a crash or a failed cleanup are recognised by any later run: **a sweep leaves every row whose import holds a live lease alone, and otherwise completes each marked row older than 10 minutes that has its vector (the marker is stripped) and removes each one that has none**, at the start of every import, at server startup (every configured store, in the background), and on demand with the new admin tool **`memory_import_sweep`** (`older_than_minutes`, default 10; full profile only, so the tool count is now 44). Until then a marked row is not a memory: `list`, the keyword and semantic search legs, hydration, `get`, `memory_export`, the graph viewer's `/api/memories` (and, through `list`/`get`, `/api/graph` and `/api/memories/{id}`), stats (which report it separately as `import_pending`), tags, tag validation, the hierarchy, related-metadata batches, duplicate pairs, link/boost/update, `backfill_tags`, merge-import dedupe and the R2 image migration all skip it, and neither the snapshot repair pass, the embedding integrity audit nor a rebuild embeds it. `import_attempt` is now a reserved metadata key (a create or update carrying it is rejected; an import strips it). A document's tag follows its resolved project: explicit, else `metadata.project`, else a configured project tag. The graph viewer's issue and todo filters, and the digest's todo and issue buckets, accept any `<project>/issues`, `<project>/todos` or bare tag, the legacy `memora/...` ones included, as well as `metadata.type`.
